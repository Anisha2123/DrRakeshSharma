...
const INSURERS: Partner[] = [
  /* ── Insurance companies ── */
  { name: "Aditya Birla Health Insurance", short: "AB", type: "Insurance", domain: "adityabirlahealthinsurance.com", color: "#e8003d" },
  { name: "Tata AIG",                      short: "TA", type: "Insurance", domain: "tataaig.com",                   color: "#0a3f7f" },
  { name: "New India Insurance",            short: "NI", type: "Insurance", domain: "newindia.co.in",               color: "#004b8d" },
  { name: "Oriental Insurance",             short: "OI", type: "Insurance", domain: "orientalinsurance.org.in",     color: "#0066cc" },
  { name: "United India Insurance",         short: "UI", type: "Insurance", domain: "uiic.co.in",                  color: "#003087" },
  { name: "Bajaj Allianz",                  short: "BA", type: "Insurance", domain: "bajajallianz.com",            color: "#003da5" },
  { name: "Star Health Insurance",          short: "SH", type: "Insurance", domain: "starhealth.in",               color: "#e4002b" },
  { name: "SBI General",                    short: "SB", type: "Insurance", domain: "sbigeneral.in",               color: "#22409a" },
  { name: "Future Generali",                short: "FG", type: "Insurance", domain: "futuregenerali.in",           color: "#e4002b" },
  { name: "Max Bupa (Niva Bupa)",           short: "MB", type: "Insurance", domain: "nivabupa.com",                color: "#e4002b" },
  { name: "National Insurance",             short: "NA", type: "Insurance", domain: "nationalinsurance.nic.co.in", color: "#004b8d" },
  { name: "Universal Sompo General",        short: "US", type: "Insurance", domain: "universalsompo.com",          color: "#003da5" },

  /* ── TPA companies ── */
  { name: "Medi Assist TPA",     short: "MA", type: "TPA", domain: "mediassist.in",         color: "#0082c8" },
  { name: "Vipul Medcorp TPA",   short: "VM", type: "TPA", domain: "vipulmedcorp.com",       color: "#003da5" },
  { name: "Raksha TPA",          short: "RK", type: "TPA", domain: "raksha-tpa.com",         color: "#e4002b" },
  { name: "Health India TPA",    short: "HI", type: "TPA", domain: "healthindiatpa.com",      color: "#0082c8" },
  { name: "MD India TPA",        short: "MD", type: "TPA", domain: "mdindia.com",             color: "#003da5" },
  { name: "GHPL TPA",            short: "GH", type: "TPA", domain: "ghplgroup.com",           color: "#2CCED1" },
  { name: "FHPL TPA",            short: "FH", type: "TPA", domain: "fhpl.net",               color: "#003da5" },
  { name: "Park Mediclaim TPA",  short: "PM", type: "TPA", domain: "parkmediclaim.co.in",     color: "#0082c8" },
  { name: "East West TPA",       short: "EW", type: "TPA", domain: "eastwesttpa.com",         color: "#e4002b" },
  { name: "Heritage TPA",        short: "HE", type: "TPA", domain: "heritagehealthtpa.com",   color: "#003da5" },
  { name: "E-Meditek TPA",       short: "EM", type: "TPA", domain: "emeditek.com",            color: "#0082c8" },
  { name: "Ericson TPA",         short: "ER", type: "TPA", domain: "ericson.in",              color: "#003da5" },
  { name: "Paramount TPA",       short: "PA", type: "TPA", domain: "paramounttpa.com",        color: "#e4002b" },

  /* ── Government Schemes ── */
  {
    name: "RGHS — Rajasthan Govt Health Scheme",
    short: "RG", type: "Govt Scheme",
...
function CompanyLogo({ domain, short, color }: { domain: string; short: string; color: string }) {
  const [loaded, setLoaded]   = useState(false);
  const [errored, setErrored] = useState(false);
  if (errored) return <FallbackAvatar short={short} color={color} />;
  return (
    <div className="logo-box" style={{ width: 40, height: 40, borderRadius: 10, background: "#fff", border: "1px solid rgba(26,48,64,.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.06)", transition: "box-shadow .2s, transform .25s cubic-bezier(.34,1.56,.64,1)" }}>
      {!loaded && <FallbackAvatar short={short} color={color} />}
      <img src={logoUrl(domain)} alt={short} width={26} height={26}
        style={{ display: loaded ? "block" : "none", objectFit: "contain", borderRadius: 4 }}
...
function SpecialCard({ partner, inView, delay }: { partner: Partner; inView: boolean; delay: number }) {
  ...
  return (
    ...
    >
      ...
      <CompanyLogo domain={partner.domain} short={partner.short} color={partner.color} />
...
export default function InsuranceEmpanelment() {
  ...
  /* split into regular and special for layout */
    ...
    <>
      <style>{`
        ...
        .ins-root::after {
          ...
          width: 800px; height: 800px; top: -300px; left: -220px;
          border-radius: 50%;
          background: radial-gradient(circle, rgba(44,206,209,.09) 0%, transparent 65%);
          pointer-events: none; z-index: 0;
          animation: rootGlow 10s ease-in-out infinite;
        }
        ...
        .ins-grid-bg {
          ...
          background-image:
            linear-gradient(rgba(44,206,209,.055) 1px, transparent 1px),
            linear-gradient(90deg, rgba(44,206,209,.055) 1px, transparent 1px);
          background-size: 60px 60px;
        }
        .ins-blob {
          position: absolute; border-radius: 50%; pointer-events: none; z-index: 0;
          width: 520px; height: 520px; bottom: -170px; right: -130px;
          background: radial-gradient(circle, rgba(255,138,91,.1) 0%, transparent 65%);
          filter: blur(70px);
        ...
        .ins-stats-row {
          ...
          box-shadow: 0 4px 18px rgba(44,206,209,.1);
          opacity: 0; transition: opacity .8s ease .5s;
        }
        .ins-stats-row.in { opacity: 1; }
        .ins-stat-cell {
          padding: 14px 22px; text-align: center;
          border-right: 1px solid rgba(44,206,209,.12);
          position: relative;
        }
        ...
        .ins-filter-btn {
          ...
          cursor: pointer; border: 1px solid rgba(44,206,209,.25);
          background: #fff; color: rgba(26,48,64,.55);
        ...
        /* special tab — orange accent for Govt Schemes */
        ...
        .ins-section-label {
          display: flex; align-items: center; gap: 12px; margin-bottom: 16px; margin-top: 8px;
        }
        ...
        /* ── SPECIAL CARDS GRID — wider, 3 col ── */
        .ins-special-grid {
          display: grid;
        ...
        .ins-card {
          ...
          padding: 16px 18px;
          background: #fff;
          border: 1px solid rgba(44,206,209,.13);
          border-radius: 14px;
          opacity: 0; transform: translateY(18px) scale(.97);
          transition: opacity .5s ease, transform .5s cubic-bezier(.22,.68,0,1.18), border-color .2s, box-shadow .2s;
          overflow: hidden;
        }
        ...
        @media (max-width: 440px) {
          .ins-cards-grid { grid-template-columns: 1fr; }
        }
      `}</style>
      ...
      <section className="ins-root">
        ...
        <div className="ins-inner">
          ...
          {/* result count */}
          {(search || filter !== "all") && (
            <div className="ins-result-count">
              Showing {filtered.length} of {grandTotal} partners
            </div>
          )}

          {/* empty state */}
          {filtered.length === 0 && (
            <div className="ins-empty" style={{ gridColumn: "1/-1" }}>No partners found for "{search}"</div>
          )}

          {/* ── GOVT SCHEMES + ACCREDITATION — special wide cards ── */}
          {special.length > 0 && (
            <>
              ...
              <div className="ins-special-grid">
                {special.map((ins, i) => {
                  const delay = gridIn ? Math.min(i * 60, 400) : 0;
                  return <SpecialCard key={ins.name} partner={ins} inView={gridIn} delay={delay} />;
                })}
              </div>
          ...
          {regular.length > 0 && (
            <div className="ins-cards-grid">
              {regular.map((ins, i) => {
                const delay = gridIn ? Math.min(i * 45, 700) : 0;
                ...
                return (
                  ...
                  >
                    <CompanyLogo domain={ins.domain} short={ins.short} color={ins.color} />
                    <div className="ins-card-info">
                    ...
                    <div className="ins-card-sno">
                      {String(INSURERS.indexOf(ins) + 1).padStart(2, "0")}
                    </div>
                  </div>
                );
              })}
