...
const REASONS = [
  ...
  },
  {
    ...
    color: T.orange,
    img:   "/TrustedandKindApproach.png",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" width="18" height="18">
...
/* ─── STAT COUNTER CELL ──────────────────────────────────────── */
function StatCell({ n, suffix, label, color, trigger, delay, borderRight, borderBottom }: {
  n: number; suffix: string; label: string; color: string;
  trigger: boolean; delay: number;
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        color: hov ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.42)",
        marginTop: "6px", transition: "color 0.35s",
      }}>
      ...
      <div style={{
        ...
        opacity: hov ? 1 : 0.4, transition: "opacity 0.35s",
      }} />
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: "20px", overflow: "hidden", cursor: "default",
    ...
    >
      <div style={{ position: "relative", height: "160px", overflow: "hidden" }}>
        <img src={img} alt={title} style={{ width: "100%", height: "100%", objectFit: "cover", transform: hov ? "scale(1.07)" : "scale(1)", transition: "transform 0.6s cubic-bezier(.22,1,.36,1)", filter: hov ? "brightness(0.65)" : "brightness(0.72)" }} />
        <div style={{ position: "absolute", inset: 0, background: hov ? `linear-gradient(to bottom,${color}55 0%,rgba(0,0,0,0.55) 100%)` : "linear-gradient(to bottom,transparent 20%,rgba(0,0,0,0.5) 100%)", transition: "background 0.45s" }} />
        <div style={{ position: "absolute", top: "14px", left: "14px", width: "36px", height: "36px", borderRadius: "10px", background: hov ? color : "rgba(255,255,255,0.92)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", color: hov ? T.white : color, border: hov ? "none" : `1px solid ${color}30`, transition: "all 0.38s", transform: hov ? "rotate(-8deg) scale(1.08)" : "rotate(0) scale(1)" }}>{icon}</div>
        <span style={{ position: "absolute", top: "14px", right: "14px", fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: "0.78rem", fontStyle: "italic", color: "rgba(255,255,255,0.6)" }}>0{idx + 1}</span>
        <h3 style={{ position: "absolute", bottom: "14px", left: "16px", right: "16px", fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: "1.12rem", fontWeight: 700, color: T.white, margin: 0, lineHeight: 1.2, letterSpacing: "-0.2px", textShadow: "0 2px 8px rgba(0,0,0,0.3)" }}>{title}</h3>
      </div>
      <div style={{ padding: "16px 18px 20px" }}>
        <p style={{ fontFamily: "'DM Sans',system-ui,sans-serif", fontSize: "0.82rem", lineHeight: 1.75, color: hov ? T.mid : "#666", margin: 0, fontWeight: 300, transition: "color 0.35s" }}>{body}</p>
        <div style={{ marginTop: "14px", height: "2px", borderRadius: "1px", background: `linear-gradient(90deg,${color} 0%,transparent 100%)`, transform: hov ? "scaleX(1)" : "scaleX(0.35)", transformOrigin: "left", transition: "transform 0.4s cubic-bezier(.22,1,.36,1)" }} />
      </div>
    </div>
  );
}
...
export default function WhyChooseDrSharma() {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) { setVis(true); setTimeout(() => setTrigger(true), 300); }
    }, { threshold: 0.06 });
  ...
  return (
    <>
      <style>{`
        ...
        /* stats 2×2 grid — always stays inside its container */
        .wc-stats-grid {
          display: grid;
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ marginBottom: "clamp(2.5rem,5vw,4.5rem)" }}>
            <div className="wc-ey" style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "18px" }}>
              ...
              <span style={{ fontSize: "0.65rem", textTransform: "uppercase", letterSpacing: "0.32em", color: T.teal, fontWeight: 600 }}>Professional Highlights & Philosophy</span>
            </div>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: "24px", flexWrap: "wrap" }}>
              <h2 className="wc-h2" style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: "clamp(2.4rem,5.5vw,4.4rem)", fontWeight: 700, lineHeight: 0.95, letterSpacing: "-2px", margin: 0, color: T.dark }}>
                Why Choose<br />
                <em style={{ color: T.orange, fontStyle: "italic" }}>Dr. Sharma?</em>
              </h2>
              <p className="wc-sub" style={{ maxWidth: "340px", fontSize: "0.9rem", lineHeight: 1.85, color: T.mid, fontWeight: 300, margin: 0 }}>
          ...
          >
            {/* Left — doctor image */}
            <div className="wc-img" style={{ borderRadius: "24px", overflow: "hidden", position: "relative", minHeight: "420px" }}>
              <img src="/drrakeshsharma.jpg" alt="Dr. Rakesh Sharma" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", filter: "brightness(0.75)" }} />
              ...
              <div style={{ position: "absolute", top: "20px", right: "20px", background: "rgba(255,255,255,0.12)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: "14px", padding: "10px 16px" }}>
            ...
            {/* Right — 2×2 stats grid + quote */}
            <div style={{ display: "flex", flexDirection: "column", gap: "clamp(1rem,2vw,1.5rem)" }}>

              {/* ── STATS: 2×2 grid, never overflows ── */}
              <div style={{ borderRadius: "20px", overflow: "hidden", background: T.dark, flex: 1, position: "relative" }}>
                ...
                <p style={{ fontFamily: "'DM Sans',system-ui,sans-serif", fontSize: "0.6rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.26em", color: "rgba(255,255,255,0.3)", padding: "18px 20px 0", margin: 0 }}>
                  Professional Highlights
                </p>

                {/* 2×2 grid — top-left, top-right, bottom-left, bottom-right */}
                <div className="wc-stats-grid" style={{ padding: "8px 0 12px" }}>
                  {STATS.map((s, i) => (
                    <StatCell
                      key={i}
                      {...s}
                      trigger={trigger}
                      delay={0.2 + i * 0.09}
                      borderRight={i % 2 === 0}        /* left cells get right border */
                      borderBottom={i < 2}             /* top row cells get bottom border */
                    />
                  ))}
                </div>
              </div>
              ...
              {/* quote card */}
              <div style={{ borderRadius: "20px", background: T.white, border: `1px solid ${T.border}`, padding: "24px 24px 22px", position: "relative", overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.05)" }}>
                <div style={{ position: "absolute", top: 0, right: "24px", width: "32px", height: "2px", background: T.orange, borderRadius: "0 0 2px 2px" }} />
                <div style={{ position: "absolute", top: "12px", left: "18px", fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: "5rem", lineHeight: 1, color: `${T.orange}14`, fontWeight: 700, userSelect: "none" }}>"</div>
                <p style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: "clamp(1rem,1.8vw,1.18rem)", fontWeight: 600, lineHeight: 1.55, color: T.ink, margin: "0 0 14px", position: "relative" }}>
          ...
          <div style={{ marginBottom: "clamp(1.5rem,3vw,2.5rem)" }}>
            <p style={{ fontSize: "0.62rem", textTransform: "uppercase", letterSpacing: "0.28em", color: T.muted, fontWeight: 600, margin: "0 0 16px" }}>Why Choose Dr. Sharma</p>
            <div className="wc-cards-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "14px" }}>
              {REASONS.map((r, i) => <ReasonCard key={i} {...r} idx={i} vis={vis} />)}
            </div>
          </div>

          {/* CTA STRIP */}
          <div style={{ borderRadius: "20px", background: T.dark, padding: "clamp(20px,3vw,28px) clamp(20px,3.5vw,32px)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "20px", flexWrap: "wrap", position: "relative", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.16)" }}>
            ...
            <div style={{ display: "flex", gap: "10px", position: "relative", flexShrink: 0 }}>
              <button
                style={{ fontFamily: "'DM Sans',system-ui,sans-serif", fontSize: "0.84rem", fontWeight: 600, letterSpacing: "0.05em", padding: "12px 26px", borderRadius: "100px", border: "none", background: T.orange, color: T.white, cursor: "pointer", boxShadow: `0 6px 24px ${T.orange}40`, transition: "all 0.3s cubic-bezier(.22,1,.36,1)" }}
